...
void Radio::initialize()
{
	...
	declareOutput("TX energy");
	declareOutput("RX energy");
	declareOutput("Sleep energy");
	declareOutput("Transition energy");

	declareOutput("TX time");
	declareOutput("RX time");
	declareOutput("Sleep time");
	declareOutput("Transition time");
...
void Radio::handleMessage(cMessage * msg)
{
	...
	switch (msg->getKind()) {
		...
		case WC_SIGNAL_START:{
			...
			receivedSignals.push_front(newSignal);
			updateTotalPowerReceived(newSignal.power_dBm, newSignal.ID);
		...
		case WC_SIGNAL_END:{
			...
			for (it1 = receivedSignals.begin(); it1 != receivedSignals.end(); it1++) {
				...
				double BER = SNR2BER(it1->power_dBm - it1->currentInterference);
				it1->bitErrors += bitErrors(BER, numOfBits, maxErrorsAllowed(it1->encoding) - it1->bitErrors);
				// trace() << "BER " << BER << " numOfBits " << numOfBits << " maxAllowed " << maxErrorsAllowed(it1->encoding);
		...
		case RADIO_CONTINUE_TX:{
			trace() << "RADIO_CONTINUE_TX";
			if (!radioBuffer.empty()) {
				double timeToTxPacket = popAndSendToWirelessChannel();
				powerDrawn(TxLevel->txPowerConsumed);
				collectOutput((lastState + " energy").c_str(), "", SIMTIME_DBL(simTime() - lastTime) * (lastPower+6) / 1000);
				collectOutput((lastState + " time").c_str(), "", SIMTIME_DBL(simTime() - lastTime));
				lastTime = simTime();
				lastPower = TxLevel->txPowerConsumed;
				lastState = "TX";
...
void Radio::handleRadioControlCommand(RadioControlCommand * radioCmd)
{
	switch (radioCmd->getRadioControlCommandKind()) {
		...
		case SET_STATE:{
			 ...
			 */
			// if (radioCmd->getState() == FORCE_SLEEP) {
			// 	isForceSleep = true;
			// 	radioCmd->setState(SLEEP);
			// } else if (radioCmd->getState() == WAKEUP) {
			// 	isForceSleep = false;
			// 	break;
			// } else if (isForceSleep) break;
			...
			if ((state == TX) && !radioCmd->isSelfMessage() && (changingToState == -1)) {
				...
				trace() << "stateAfterTX " << (stateAfterTX == TX ? "TX" :
					(stateAfterTX == RX ? "RX" : "SLEEP"));
				break;
			}
			...
			collectOutput((lastState + " time").c_str(), "", SIMTIME_DBL(simTime() - lastTime));
				lastTime = simTime();
			lastPower = avgDrawnTransitionPower;
			lastState = "Transition";
			trace() << "SET STATE to " << (changingToState == TX ? "TX" :
					(changingToState == RX ? "RX" : "SLEEP")) << ", delay=" <<
					transitionDelay << ", power=" << avgDrawnTransitionPower << " stateAfterTX " << (stateAfterTX == TX ? "TX" :
					(stateAfterTX == RX ? "RX" : "SLEEP"));
...
double Radio::popAndSendToWirelessChannel()
{
	...
	begin->setDestID(macPkt->getDestination());
	...
	end->setDestID(macPkt->getDestination());
 ...
 */
void Radio::updateTotalPowerReceived(double newSignalPower, int id)
{
...
void Radio::updateTotalPowerReceived(list<ReceivedSignal_type>::iterator endingSignal)
{
	...
	for (it1 = receivedSignals.begin(); it1 != receivedSignals.end(); it1++) {
		if (it1 != endingSignal) {
			...
			newElement.src.push_back(it1->ID);
		}
	}
	newElement.startTime = simTime();
	totalPowerReceived.push_front(newElement);
