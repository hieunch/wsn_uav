...
void GAMBAC::mainAlg() {
	...
	for (int k_min=8; k_min<=100; k_min+=2) {
		...
		for (int ii=0; ii<10; ii++) {
			...
			for (int i=0; i<N; i++) {
				...
				CastaliaModule::trace2() << "RESULT" << count2 << "\tPOINT\tblue\t" << P.x() << "\t" << P.y() << "\t" << weights[i] << "\t" << clus_id[i] << "\t" << i;
			}
			for (int i=0; i<N; i++) {
				if (clus_id[i] != -1) {
					auto P = GlobalLocationService::getLocation(i);
	...
	do {
		...
		for (int u=0; u<numNodes; u++) {
			if (clus_id[u] == -1 || u == self) continue;
			for (int k=0; k<trajectories.size(); k++) {
	...
	for (int l : Anew) {
		...
		while (!queue.empty()) {
			...
			for (int v : graph.getAdjExceptSink(u)) {
				...
				if (alt < 1.1*d2CH[v] && calculateCHEnergy(l, computeWeight(representSet[l])) > calculateCHEnergy(clus_id[v], computeWeight(representSet[clus_id[v]]))) {
					d2CH[v] = d2CH[u] + graph.getLength(u,v);
					representSet[clus_id[v]].remove(v);
					clus_id[v] = l;
					nextHop[v] = u;
	...
	*/
	...
	// for (int u=0; u<N; u++) {
	// 	if (clus_id[u] >= 0) debugLine(location(u), location(clus_id[u]), "black");
	// }
...
vector<int> GAMBAC::TZ_sample(vector<int>W, double s) {
...
void GAMBAC::growBalls(vector<int> CHSet){
	...
	while (!queue.empty()) {
		...
		int u = queue.top();
		queue.pop();
		for (int i=0; i<1; i++) trace() << "u " << u << " d[u] " << d2CH[u];
		...
		for (int v : graph.getAdjExceptSink(u)) {
			...
			// else if ((abs(alt - d2CH[v]) < EPSILON) && E_tmp[nextHop[v]] < E_tmp[u]){// && (calculateCHEnergy(clus_id[u], computeWeight(representSet[clus_id[u]])) > calculateCHEnergy(clus_id[v], computeWeight(representSet[clus_id[v]])))) {
			// 	// representSet[clus_id[v]].remove(v);
			// 	clus_id[v] = clus_id[u];
			// 	nextHop[v] = u;
			// 	// representSet[clus_id[v]].push_back(v);
			// }
		}
	}
	int totalSize = 0;
...
void GAMBAC::growBalls2(vector<int> CHSet){
	// if (roundNumber == 169) for (int i=0; i<10; i++) trace1() << "growBalls Asize " << A.size();
	for (int u : CHSet) {
		d2CH[u] = 0;
		representSet[u].clear();
	...
	while (!queue.empty()) {
		...
		for (int v : graph.getAdjExceptSink(u)) {
			...
			// if (roundNumber == 169) for (int i=0; i<10; i++) trace1() << "if";
			if (alt - d2CH[v] < -EPSILON && computeWeight(representSet[clus_id[u]]) + weights[v] > w_max[clus_id[u]]) {
				trace1() << "v " << v << " clus_id[u] " << clus_id[u] << " weight " << (computeWeight(representSet[clus_id[u]]) + weights[v]) << " threshold " << w_max[clus_id[u]];
				stringstream ss;
				for(int i : representSet[clus_id[u]]) {
					ss << i << " ";
				}
			...
			// 	queue.push(v);
			// }
			// else if ((abs(alt - d2CH[v]) < EPSILON) && (calculateCHEnergy(clus_id[u], computeWeight(representSet[clus_id[u]])) > calculateCHEnergy(clus_id[v], computeWeight(representSet[clus_id[v]]))) && computeWeight(representSet[clus_id[u]]) + weights[v] <= w_max[clus_id[u]]) {
			// 	// representSet[clus_id[v]].remove(v);
			// 	clus_id[v] = clus_id[u];
			// 	nextHop[v] = u;
			// 	// representSet[clus_id[v]].push_back(v);
			// }
			if  (calculateCHEnergy(clus_id[u], computeWeight(representSet[clus_id[u]])) > calculateCHEnergy(clus_id[v], computeWeight(representSet[clus_id[v]]))) {
				clus_id[v] = clus_id[u];
				nextHop[v] = u;
				queue.push(v);
			}
...
double GAMBAC::computeWeight(list<int> S) {
	double weight = 0;
	for (int u : S) weight += weights[u];
	return weight;
}

void GAMBAC::updateCentList() {
	for (int u=0; u<numNodes; u++) {
		if ((u == self)) continue;
		double minDist = DBL_MAX;
		for (int l : A) {
			if (!graph.hasEdge(l, u) || (l == clus_id[u])) continue;
...
double GAMBAC::computeClusterWeight(int uNode){
...
vector<int> GAMBAC::verifyFringeSet(){
	...
	for (int CH : A){
		...
		if (weight > Wt) {
			weight = 0;
			// for (int i=0; i<5; i++) trace() << "CH " << CH << " ballWeight " << weight << " w_max " << w_max[CH];
			dCompare =  &d2CH;
			priority_queue<int,vector<int>, decltype(&Comparebydistance)> queue(Comparebydistance);
...
vector<int> GAMBAC::TZ_sample2(vector<int> W, double b) {
...
void GAMBAC::recruitNewCHsAlpha(){
	...
	if (W.empty()){
		...
		for (int w : fringeSet){
			...
			if (debugRecruitProcess) CastaliaModule::trace2() << "RECRUIT" << countAdjusment << "\tPOINT\tgreen\t" << P.x() << "\t" << P.y() << "\t" << weights[w] << "\t" << clus_id[w] << "\t" << w; 
	...
	for (int w : A2){
		auto P = GlobalLocationService::getLocation(w);
		if (debugRecruitProcess) CastaliaModule::trace2() << "RECRUIT" << countAdjusment << "\tPOINT\tred\t" << P.x() << "\t" << P.y() << "\t" << weights[w] << "\t" << clus_id[w] << "\t" << w;
	};
	// growBalls(A2);
	clearData();
	growBalls(A);
...
void GAMBAC::clearData(){
	for (int u : graph.getNodesExceptSink()){
		...
		// for (int i=0; i<10; i++) trace1() << "clus_id";
		clus_id[u] = -1;
...
int GAMBAC::constructClusters(int k){
	...
	for (int i=0; i<N; i++) {
		// if ((w_max[i] >= w_min) && distance(i, sinkId) <= L_max/2 && !isCH[i]) 
	...
	do {
		...
		*/
		...
		for (int i=0; i<N; i++) {
			auto P = GlobalLocationService::getLocation(i);
			if (debugRecruitProcess) CastaliaModule::trace2() << "RECRUIT" << countAdjusment << "\tPOINT\tblue\t" << P.x() << "\t" << P.y() << "\t" << weights[i] << "\t" << clus_id[i] << "\t" << i;
		}
		for (int i=0; i<N; i++) {
			if (clus_id[i] != -1) {
				auto P = GlobalLocationService::getLocation(i);
...
vector<int> GAMBAC::sortedCHVector() {
	vector<int> CHvector;
    if (A.empty()) {
        return CHvector;
    }
