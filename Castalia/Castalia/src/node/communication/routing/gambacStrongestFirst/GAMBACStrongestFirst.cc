...
int GAMBACStrongestFirst::selectCHsAndBuildDFT(double f){
	...
	for (int new_CH : sorted_idx) {
		...
		routing_trace() << "newCH " << new_CH << ", E = " << E_tmp[new_CH] << ", max_weight = " << w_max[new_CH];
		...
		for (int i=0; i<N; i++) {
			if (i == sinkId) continue;
			if (clus_id[i] != -1) isSelected[i] = true;
		}
	}
	clus_id[self] = -1;
...
void GAMBACStrongestFirst::growBallsPercent(vector<int> CHSet, double perclus_id){
	...
	while (!queue.empty()) {
		...
		for (int v : graph.getAdjExceptSink(u)) {
			...
			if (alt - distanceToCH[v] < -EPSILON && rxEnergy(T_new) + txEnergy(T_new,D2UAV) <= E_tmp[clus_id[u]] * perclus_id){
				T[clus_id[u]] = T_new;
