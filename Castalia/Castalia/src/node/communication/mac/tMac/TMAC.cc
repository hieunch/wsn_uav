...
void TMAC::timerFiredCallback(int timer)
{
	switch (timer) {
		...
		case FRAME_START:{
			...
			if (!isSleep && (macState == MAC_STATE_SLEEP))
				toRadioLayer(createRadioCommand(SET_STATE, RX));
			if (!isSleep && (macState == MAC_STATE_SLEEP || macState == MAC_STATE_ACTIVE
...
void TMAC::fromNetworkLayer(cPacket * netPkt, int destination)
{
	...
	TMacPacket *macPkt = new TMacPacket(netPkt->getName(), MAC_LAYER_PACKET);
	...
	if (bufferPacket(macPkt)) {	// this is causing problems
		if (TXBuffer.size() == 1) {
			checkTxBuffer();
			cMessage *netCmd = new cMessage("TXBUFFER1", NETWORK_CONTROL_COMMAND);
...
void TMAC::resetDefaultState(const char *descr)
{
	...
	if (activationTimeout <= getClock()) {
		trace() << "activationTimeout " << activationTimeout << " getClock() " << getClock();
		if (disableTAextension) {
			toRadioLayer(createRadioCommand(SET_STATE, SLEEP));
			setMacState(MAC_STATE_SLEEP, "active period expired (SMAC)");
		} else {
...
void TMAC::fromRadioLayer(cPacket * pkt, double RSSI, double LQI)
{
	...
	if (destination != SELF_MAC_ADDRESS && destination != BROADCAST_MAC_ADDRESS) {
		...
		cMessage *netCmd = new cMessage("EXT", NETWORK_CONTROL_COMMAND);
		toNetworkLayer(netCmd);
...
void TMAC::carrierIsClear()
{
	switch (macState) {
		...
		case MAC_CARRIER_SENSE_BEFORE_SLEEP:{
			...
			collectOutput("Activation time", "", SIMTIME_DBL(getClock() - currentFrameStart));
			break;
		}
	}
}
...
{
	trace() << "performCarrierSense " << newState;
...
int TMAC::handleControlCommand(cMessage * msg)
{
	string type(msg->getName());
	trace() << "handleControlCommand " << type;
	if (type.compare("SLEEP") == 0) {
		// listenTimeout = 0.0001;
		// isSleep = true;
		// for (int i=1; i<=10; i++) cancelTimer(i);
		// setMacState(MAC_STATE_SLEEP, "cmd from network layer");
	} else {
		// listenTimeout = 15;
		isSleep = false;
		// macState = MAC_STATE_SETUP;
