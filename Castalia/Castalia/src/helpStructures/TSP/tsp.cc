...
void TSP::readCities(){
	...
	for (int i=0; i<realIds.size(); i++) {
		Point p = GlobalLocationService::getLocation(realIds[i]);
		struct City c = {p.x_, p.y_};
		cities.push_back(c);
	}
};

double TSP::get_distance(struct TSP::City c1, struct TSP::City c2) {
	/////////////////////////////////////////////////////
	// Calculate distance between c1 and c2
	/////////////////////////////////////////////////////
	double dx = pow((float)(c1.x - c2.x), 2);
	double dy = pow((float)(c1.y - c2.y), 2);
	return (sqrt(dx + dy));
};
