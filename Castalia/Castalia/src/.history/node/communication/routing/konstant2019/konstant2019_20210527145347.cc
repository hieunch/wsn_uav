...
vector<vector<int>> konstant2019::partitionIntoSectors() {
	for (int ii=0; ii<0; ii++) trace1() << "partitionIntoSectors";
	...
	for (int ii=0; ii<0; ii++) trace1() << "P_total" << P_total;

	for (int ii=0; ii<0; ii++) trace1() << "partitionIntoSectors 1";
    ...
    for (int i = 0; i < numNodes; i++) {
		...
		for (int ii=0; ii<0; ii++) trace1() << "angle " << sinkId << " " << i0 << " " << i << " " << angle_i;
	...
	// trace1() << ss.str();
	for (int ii=0; ii<0; ii++) trace1() << "partitionIntoSectors sorted";
	...
	for (int ii=0; ii<0; ii++) trace1() << "partitionIntoSectors RaySectors";
	vector<vector<int>> RaySectors (numUAVs);
	for (int k=0; k<numUAVs; k++) {
		vector<int> sector = Sectors[k];
		for (int ii=0; ii<0; ii++) trace1() << "RaySectors " << k << " size " << sector.size();
		int u0 = sector[0];
		for (int ii=0; ii<0; ii++) trace1() << "u0 " << u0;
		...
		for (int ii=0; ii<0; ii++) trace1() << "maxAngle " << maxAngle << " ray2Angle " << ray2Angle << " ray4Angle " << ray4Angle;
		for (int u : sector) {
			Angle uAngle = G::angle(sinkLocation, location(u0), sinkLocation, location(u));
			for (int ii=0; ii<0; ii++) trace1() << "u " << u << " uAngle " << uAngle;
	...
	for (auto sector : Sectors) {
		for (int ii=0; ii<0; ii++) trace1() << "sector " << sid << " size " << sector.size();
		stringstream ss;
	...
	for (auto sector : RaySectors) {
		for (int ii=0; ii<0; ii++) trace1() << "raysector " << sid << " size " << sector.size();
		stringstream ss;
		if (!sector.empty()) for(int l : sector) {
			ss << l << " ";
		}
	...
	for (int ii=0; ii<0; ii++) trace1() << "partitionIntoSectors done";
	return RaySectors;
}

vector<vector<int>> konstant2019::basicToursPlanning() {
	vector<vector<int>> Sectors = partitionIntoSectors();
	for (int ii=0; ii<0; ii++) trace1() << "basicToursPlanning";
	...
	for (int k=0; k<numUAVs; k++) {
		...
		for (int ii=0; ii<0; ii++) trace() << "basicToursPlanning 1";
		if (Sectors[k].size() >= 1) {
			...
			if (Sectors[k].size() >= 2) {
				int i = 1;
				for (int ii=0; ii<0; ii++) trace() << "basicToursPlanning 2";
				int n_min = 0;
				int n_max = Sectors[k].size()-1;
				int n_mid;
				do {
					for (int ii=0; ii<0; ii++) trace() << i << " " << Sectors[k][i];
					n_mid = (n_min+n_max)/2;
					innerPoint.clear();
					innerPoint.push_back(location(sinkId));
	...
	for (int k=0; k<numUAVs; k++) {
		for (int ii=0; ii<0; ii++) trace() << "tour " << k << " size " << basicTours[k].size();
		for (int ii=0; ii<0; ii++) trace() << " length " << GlobalLocationService::convexHullPerimeter(basicTours[k]);
		stringstream ss;
	...
	for (int ii=0; ii<0; ii++) trace() << "basicToursPlanning done";
...
void konstant2019::findEnergyEfficientSolution() {
	...
	for (int ii=0; ii<0; ii++) trace1() << "findEnergyEfficientSolution";
	...
	for (int ii=0; ii<0; ii++) trace() << "findEnergyEfficientSolution 0";
	int retries = 2;
	int maxIter = 200;
	double cur_cnsmptn = DBL_MAX;//calculateConsumption(finalTours);
	for (int i=0; i<retries; i++) {
		for (int j=0; j<maxIter; j++) {
			for (int ii=0; ii<0; ii++) trace1() << "iter " << j << " retry " << i;
			// if (j%10 == 0) trace1() << "iter " << j;
			int rdm;
			for (int ii=0; ii<0; ii++) trace() << "findEnergyEfficientSolution 1";
			do {
			...
			for (int ii=0; ii<0; ii++) trace() << "findEnergyEfficientSolution 2";
			bool feasible;
			do {
			...
			for (int ii=0; ii<0; ii++) trace() << "findEnergyEfficientSolution 3";
	...
	// trace1() << str0;
	for (int ii=0; ii<0; ii++) trace() << "findEnergyEfficientSolution done";
}

double konstant2019::calculateConsumption(vector<vector<int>> tours) {
	for (int ii=0; ii<0; ii++) trace() << "calculateConsumption";
	A.clear();
