...
void route::print(const problem& input) const {
	std::vector<int> path;
	...
	path.push_back(input.realIds[0]);
//	cout << visits.size() << " customers, distance = " << distance << " : " << GlobalLocationService::calculatePathLength(path);
	
	for(list<int>::const_iterator it = visits.begin(); it != visits.end(); ++it){
		cout << input.realIds[*it];
	}
